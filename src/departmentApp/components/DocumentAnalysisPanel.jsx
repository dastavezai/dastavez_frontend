...
import {
  ...
  List, ListItem, ListIcon, Tag, Wrap, WrapItem, Progress, Tooltip
...
const DocumentAnalysisPanel = ({ scanResults, formatMetadata, ocrConfidence }) => {
          ...
          {ocrConfidence !== null && ocrConfidence !== undefined && (
            <Box bg={cardBg} p={3} borderRadius="md" border="1px solid" borderColor={borderColor}>
              <HStack spacing={2} mb={2}>
                <Icon as={FaFileAlt} color={ocrConfidence >= 80 ? 'green.400' : ocrConfidence >= 50 ? 'orange.400' : 'red.400'} />
                <Text fontSize="xs" fontWeight="bold" textTransform="uppercase" color={mutedColor}>OCR Quality</Text>
                <Tooltip label={ocrConfidence >= 80 ? 'High quality extraction — text is reliable' : ocrConfidence >= 50 ? 'Moderate quality — verify important sections' : 'Low quality — consider re-uploading a clearer image'}>
                  <Badge colorScheme={ocrConfidence >= 80 ? 'green' : ocrConfidence >= 50 ? 'orange' : 'red'} variant="solid" fontSize="2xs" cursor="help">
                    {ocrConfidence >= 80 ? 'Excellent' : ocrConfidence >= 60 ? 'Good' : ocrConfidence >= 40 ? 'Fair' : 'Poor'}
                  </Badge>
                </Tooltip>
              </HStack>
              <HStack spacing={2} align="center">
                <Text fontSize="lg" fontWeight="bold" color={ocrConfidence >= 80 ? 'green.400' : ocrConfidence >= 50 ? 'orange.400' : 'red.400'}>
                ...
                <Box flex="1">
                  <Progress
                    ...
                    size="sm"
                    borderRadius="full"
                    colorScheme={ocrConfidence >= 80 ? 'green' : ocrConfidence >= 50 ? 'orange' : 'red'}
                  />
                </Box>
              </HStack>
              {ocrConfidence < 60 && (
                <Text fontSize="2xs" color="red.400" mt={1}>Upload a clearer image for better results</Text>
              )}
            </Box>
          )}
