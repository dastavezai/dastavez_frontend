...
const EditableDesignPreview = ({ config, isFullScreen, onToggleFullScreen }) => {
  ...
  const getImageStyle = (img) => {
    ...
    if (img.placement === 'top-full-width' || img.isWatermark) return null;
    const offX = img.offsetX || 0;
    const offY = img.offsetY || 0;
    const positions = {
      'top-left':     { top: marginTop + 4 + offY,    left: marginLeft + 4 + offX },
      'top-center':   { top: marginTop + 4 + offY,    left: '50%', transform: `translateX(calc(-50% + ${offX}px))` },
      'top-right':    { top: marginTop + 4 + offY,    right: marginRight + 4 - offX },
      'bottom-left':  { bottom: marginBottom + 4 - offY, left: marginLeft + 4 + offX },
      'bottom-center':{ bottom: marginBottom + 4 - offY, left: '50%', transform: `translateX(calc(-50% + ${offX}px))` },
      'bottom-right': { bottom: marginBottom + 4 - offY, right: marginRight + 4 - offX },
      'center':       { top: '50%', left: '50%', transform: `translate(calc(-50% + ${offX}px), calc(-50% + ${offY}px))` },
    };
    return {
      position: 'absolute',
  ...
  const hasTopStamp = (c.images || []).some(img => img.data && img.placement === 'top-full-width');
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(c.images || []).filter(img => img.data && img.placement === 'top-full-width').map((img, idx) => (
          <Box
            key={`top-stamp-${idx}`}
            w="100%"
            textAlign={img.stampAlign || 'center'}
            px={`${marginLeft}px`}
            pt={`${marginTop}px`}
            pb={`${img.stampMarginBottom ?? 8}px`}
            position="relative"
            zIndex={1}
          >
            <Image
        ...
        {(c.images || []).map((img, idx) => {
          if (!img.data || img.isWatermark || img.placement === 'top-full-width') return null;
          const imgStyle = getImageStyle(img);
          if (!imgStyle) return null;
          return <Image key={idx} src={img.data} alt={img.label || 'Stamp'} {...imgStyle} />;
        })}

        
        {c.headerText && (
          <Box
            pt={hasTopStamp ? 0 : `${marginTop}px`}
            px={`${marginLeft}px`}
        ...
        <Box
          pt={c.headerText ? 2 : hasTopStamp ? 0 : `${marginTop}px`}
                        ...
                        {(designForm.config.images || []).length > 0 && (
                          <VStack spacing={3} align="stretch" w="100%">
                            {(designForm.config.images || []).map((img, idx) => (
                              <Box key={idx} p={2} borderWidth="1px" borderRadius="md" bg={sectionBg}>
                                <HStack justify="space-between" mb={2}>
                                  ...
                                  <IconButton
                                    ...
                                    variant="ghost"
                                    onClick={() => {
                                      const imgs = [...(designForm.config.images || [])];
                                      imgs.splice(idx, 1);
                                      updateConfig('images', imgs);
                                    }}
                                  />
                                </HStack>
                              <FormControl mb={2}>
                                <FormLabel fontSize="2xs" fontWeight="bold">Placement Type</FormLabel>
                                <Select
                                  size="xs"
                                  value={img.placement || (img.isWatermark ? 'watermark' : 'positioned')}
                                  onChange={(e) => {
                                    const newPlacement = e.target.value;
                                    const imgs = [...(designForm.config.images || [])];
                                    imgs[idx] = { ...imgs[idx], placement: newPlacement, isWatermark: newPlacement === 'watermark' };
                                    updateConfig('images', imgs);
                                  }}
                                >
                                  <option value="positioned">📌 Corner / Fixed Position</option>
                                  <option value="top-full-width">🏛️ Top of Document (Stamp Paper)</option>
                                  <option value="watermark">🌊 Full-Page Watermark</option>
                                </Select>
                              </FormControl>

                              {(img.placement === 'top-full-width') && (
                                <SimpleGrid columns={2} spacing={2} mb={2}>
                                  <FormControl gridColumn="1 / -1">
                                    <FormLabel fontSize="2xs">Stamp Width ({img.stampWidth || 100}% of page)</FormLabel>
                                    <Slider min={20} max={100} step={5}
                                      value={img.stampWidth || 100}
                                      onChange={(v) => updateImage(idx, 'stampWidth', v)}>
                                      <SliderTrack><SliderFilledTrack bg="orange.400" /></SliderTrack>
                                      <SliderThumb boxSize={3} />
                                    </Slider>
                                  </FormControl>
                                  <FormControl>
                                    <FormLabel fontSize="2xs">Alignment</FormLabel>
                                    <Select size="xs" value={img.stampAlign || 'center'}
                                      onChange={(e) => updateImage(idx, 'stampAlign', e.target.value)}>
                                  ...
                                  <FormControl>
                                    <FormLabel fontSize="2xs">Gap Below Stamp (px)</FormLabel>
                                    <NumberInput size="xs" value={img.stampMarginBottom ?? 8} min={0} max={80}
                                      onChange={(v) => updateImage(idx, 'stampMarginBottom', Number(v))}>
                                      <NumberInputField />
                                    </NumberInput>
                                  </FormControl>
                                  <FormControl gridColumn="1 / -1">
                                    <FormLabel fontSize="2xs">Opacity</FormLabel>
                              ...
                              {(!img.placement || img.placement === 'positioned') && (
                                <SimpleGrid columns={2} spacing={2} mb={2}>
                                  <FormControl>
                                  ...
                                  <FormControl>
                                    ...
                                    <NumberInput size="xs" value={img.width || 80} min={20} max={300}
                                  ...
                                  <FormControl>
                                    ...
                                    <NumberInput size="xs" value={img.height || 80} min={20} max={300}
                              ...
                              {img.placement === 'watermark' && (
                                <SimpleGrid columns={2} spacing={2} mb={2}>
                                  <FormControl>
                                    <FormLabel fontSize="2xs">Width (px)</FormLabel>
                                    <NumberInput size="xs" value={img.width || 200} min={50} max={600}
                                      onChange={(v) => updateImage(idx, 'width', Number(v))}>
                                      <NumberInputField />
                                    </NumberInput>
                                  </FormControl>
                                  <FormControl>
                                    <FormLabel fontSize="2xs">Opacity (10–50%)</FormLabel>
                                    <Slider min={0.1} max={0.5} step={0.05}
                                      value={Math.min(img.opacity ?? 0.2, 0.5)}
                                      onChange={(v) => updateImage(idx, 'opacity', v)}>
                                      <SliderTrack><SliderFilledTrack bg="teal.400" /></SliderTrack>
                                      <SliderThumb boxSize={3} />
                                    </Slider>
