...
import {
  Box, VStack, HStack, Text, Badge, IconButton, Button, Tooltip,
  Collapse, useColorModeValue, Divider, Progress, Flex, Icon
} from '@chakra-ui/react';
import { FaCheck, FaTimes, FaEye, FaChevronDown, FaChevronUp, FaUndo } from 'react-icons/fa';
...
const SmartSuggestionsPanel = ({ suggestions = [], onApply, onDismiss, onUndo, onViewInDocument }) => {
                  ...
                  <Box px={3} pb={3}>
                    ...
                    {isDone && (
                      <HStack spacing={2} mt={2} align="center">
                        <Badge
                          colorScheme={suggestion.status === 'applied' ? 'green' : 'gray'}
                          fontSize="2xs"
                          px={2} py={0.5}
                          borderRadius="full"
                        >
                        ...
                        {onUndo && (
                          <Tooltip label="Undo — restore to pending" fontSize="xs">
                            <Button
                              size="xs"
                              colorScheme="orange"
                              variant="ghost"
                              leftIcon={<FaUndo size={10} />}
                              fontSize="2xs"
                              h="22px"
                              onClick={(e) => {
                                e.stopPropagation();
